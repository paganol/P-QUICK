...
12. `nthreads`: thread count for `ducc0` and the numba-parallel resampling / map-making kernels (`0` = all available cores). Trade off against the number of MPI ranks per node. Map-making uses a single `(npix, 3, 3)` accumulator (~3.6 GB at nside 2048) independent of `nthreads`; the per-sample trig is parallel and only the cheap scatter is serial.
